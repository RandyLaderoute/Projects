...
const Calculator = {
    //this is displays a 0 on screen
    Display_Value: '0',
    // this will hold the first oerand for any exrepssions, we set it to null for now
    First_Operand: null,
...
function Input_Digit(digit) {
    ...
    // Display_Value to the keyh that was clicked.
...
function Input_Decimal (dot) {
    ...
    if(!Calculator.Display_Value.indicates(dot)) {
...
function Handle_Operators(Next_Operator){
    ...
    //then updates the operator and exits from teh function
    ...
    }else if (operator) { //checks if operator already exist
        const Value_Now = First_Operand || 0;
        //if operator exist, property lookup is performed for the operator
        //in the Perform_Calculation object and t he function that matches the operator is executed
...
keys.addEventListener('click', (event) => {
    ...
    //ensures that AC clearsn the numbers from the Calculator
